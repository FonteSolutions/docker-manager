        ...
        switch(true) {
            ...
            case pathname == '/container-logs':
                ...
                request.on('data', function(chunk) {
                    ...
                    response.writeHeader(200, HEADER.JSON);
                    response.write(API.run('containers/' + _data.containerId + '/changes', {}, false, false, true));
                    response.end();
                });
                break;

            case pathname == '/container-top':
                var data = [];
                request.on('data', function(chunk) {
                    data.push(chunk);
                }).on('end', function() {
                    ...
                    response.writeHeader(200, HEADER.JSON);
                    response.write(API.run('containers/' + _data.containerId + '/top', {'ps_args': 'aux'}, false, false, true));
                    response.end();
                });
                break;

            case pathname == '/container-stats':
                var data = [];
                request.on('data', function(chunk) {
                    data.push(chunk);
                }).on('end', function() {
